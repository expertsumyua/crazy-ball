...
function endGame(statusGame)
{	// это означает, что игра остановлена
	game = false;
	// остановить тамер
...
function timerGame()
{
	timergame = setInterval (function()
		{	// ведём обратный отсчёт таймера
			timerBlock.innerText--;
			// если таймер достиг нуля то:
			if(timerBlock.innerText == 0)
			{	// вызываем функцию конец игры
				endGame("lose");								
			}	
		}, 1000);
}
// функция реакциишарика на мышь
function mouseBall(thisBall)
{	// Если игра еще идет, то:
	if (game)
	{	
...
{	// создаём блок див <div id="start-block"></div>
...
function createLifesBlock(curretLifesSum = 5)
{	// создаём блок div <d = 5iv id="lifes-block">
	...
	setTimeout(function()
		{	 
			...
			{		
				...
				setTimeout(function()
				{	// если шарик находится в финишной точке, то:
					...
					{	// уменьшаем количество пойманых шаров (т.к. дальше они добавятся наместо)
						...
						{	// вызвать функцию окончания Игры
							endGame("lose");
						}
					}					
				},3000);						
...
{	// соpдаём блок <div id="finish-game"></div>
...
function removeBall(thisBall)
{	//делаем шарик прозначнам	
	thisBall.style.opacity = "0";
	// удаляем шарик
	thisBall.remove();		
}
