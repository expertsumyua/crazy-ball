			...
			setTimeout(function () {	// если шарик находится в финишной точке, то:
				if ((thisBall.offsetTop == finishTop) || (thisBall.offsetLeft == finishLeft)) {
					...
					if (lifesSum) {	// удаляем блок жизней
						...
						// тут есть добавление колчество очко pointSum++, поэтому:
						// уменьшаем количество пойманых шаров (т.к. выше они увеличиваются)
						pointSum--;
						// А потом только запущу функцию функцию обработки движения мыши в шарике
						mouseBall(thisBall);
